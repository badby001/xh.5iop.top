    ...
    <div class="layui-card">
        <div class="layui-card-body">
            ...
            <div class="layui-col-md3 eleTree">
                <div class="layui-card-header1" data-step="4"
                     data-intro="您好! 在导航配置中, 您可以右键根目录, 新增->添加下级目录, 添加成功后可以在添加的目录上右键进行设置其他内容"
                     data-position="bottom">
                    <div class="layui-card-header1" data-step="5"
                         data-intro="您好! 目录添加成功后, 您可以点击您需要关联产品的目录, 然后进行第6步操作"
                         data-position="bottom">
            ...
            <div class="layui-col-md9">
                <div class="layui-card-header1" data-step="6"
                     data-intro="您好! 在第5步选择好目录后, 可以点击'关联线路', 在新窗口中勾选需要和当前目录相关的产品;"
                     data-position="bottom">
...
<script>
    layui.use(["admin", "element", "table", "form", "laydate", "okLayer", "eleTree", "okUtils", "okLayx", "introJs"], function () {
        ...
        function initPermissionTree() {
            okUtils.ajax("{{url('/sys/pages/system/navigationRead')}}", "get", null, true).done(function (response) {
                eleTree.render({
                    elem: '#permissionTree',
                    data: response.data,
                    @if(hasPower(221)) draggable: true, @endif
                    showLine: true,
                    expandOnClickNode: false,
                    renderAfterExpand: false,
                    defaultExpandedKeys: [0],
                    contextmenuList: [@if(hasPower(220))"add.async", @endif @if(hasPower(221))"edit.async", @endif @if(hasPower(223))"remove.async", @endif @if(hasPower(221)){
                        eventName: "set",
                        text: "设置"
                    }@endif],
                    indent: 10,
                ...
                eleTree.on("nodeDrag(data1)", function (d) {
                    okUtils.ajax("{{url('sys/pages/system/navigationTarget')}}", "post", {
                        'current': d.current.data.currentData,
                        'target': d.target.data.currentData,
                        '_token': '{{csrf_token()}}'
                    }, true).done(function (response) {
                        //进行排序
                        okUtils.ajax("{{url('sys/pages/system/navigationSort')}}", "post", {
                ...
                eleTree.on("nodeAppend(data1)", function (d) {
                    ...
                    okUtils.ajax("{{url('sys/pages/system/navigation')}}/", "post", {
                ...
                eleTree.on("nodeEdit(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okUtils.ajax("{{url('sys/pages/system/navigationNodeEdit')}}/" + d.data.id, "post", {
                            'title': d.value,
                            '_token': '{{csrf_token()}}'
                        }, true).done(function (response) {
                            // 异步操作必须调用之后才会修改
                ...
                eleTree.on("nodeRemove(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okUtils.ajax("{{url('sys/pages/system/navigation')}}/" + d.data.id, "DELETE", {
                            '_token': '{{csrf_token()}}'
                        }, true).done(function (response) {
                            // 异步操作必须调用之后才会修改
                            d.async();
                ...
                eleTree.on("nodeSet(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okLayer.open("编辑导航", "navigation/" + d.data.id + "/edit", "90%", "90%", null, function () {
                ...
                eleTree.on("nodeClick(data1)", function (d) {
                    //console.log(d.data.currentData);    // 点击节点对应的数据
                    // console.log(d.event);   // event对象,{eventName: "test", text: "test"}
                    // console.log(d.node);    // 点击的dom节点
                    // console.log(this);      // 与d.node相同
                    $('#navigation_id').val(d.data.currentData.id);
                    $('#classify').val(d.data.currentData.classify);
                    initPermissionTable(d.data.currentData.id, d.data.currentData.classify);
                });
                //
        ...
        function initPermissionTable(navigation_id, classify) {
            let navTable = table.render({
                elem: '#tableId',
                url: '{{url('/sys/pages/system/navigationLineRead')}}',
                where: {'navigation_id': navigation_id, 'classify': classify},
                limit: '{!! frame()['limit'] !!}',
                limits: [{!! frame()['limits'] !!}],
                title: '线路列表_{{getTime(3)}}',
                page: true,
                even: true,
                toolbar: '<div class="layui-btn-container">\n' +
                    @if(hasPower(220)) '<button class="layui-btn layui-btn-sm" lay-event="add">关联产品</button>\n' + @endif
                        @if(hasPower(222)) '<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="batchEnabled">批量启用</button>\n' +
                    '<button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="batchDisabled">批量停用</button>\n' + @endif
                        @if(hasPower(223)) '<button class="layui-btn layui-btn-sm layui-btn-danger" lay-event="batchDel">批量删除</button>\n' + @endif
                        '<button class="layui-btn layui-btn-sm layui-btn-primary" lay-event="batchRedis">刷新缓存</button>\n' +
                ...
                cols: [[
                    ...
                    {field: "navigation_name", title: "导航名称", width: 100},
                    ...
                    {field: "up_time", title: "修改时间", width: 145, sort: true},
                    {
                        title: "操作", width: 100, align: "center", fixed: "right", templet: function (d) {
                            var del = "@if(hasPower(223))<a href=\"javascript:\" title=\"删除\" lay-event=\"del\"><i class=\"layui-icon\">&#xe640;</i></a>@endif";
                            if (d.is_lock == 1) {
                                return del;
                            } else {
                                return '';
            ...
            function batchEnabled() {
                okLayer.confirm("确定要批量启用吗？", function (index) {
                    ...
                    if (idsStr) {
                        okUtils.ajax("navigationLineStart", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
            ...
            function batchDisabled() {
                okLayer.confirm("确定要批量停用吗？", function (index) {
                    ...
                    let idsStr = okUtils.tableBatchCheck(table);
                    if (idsStr) {
                        okUtils.ajax("navigationLineStop", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
                            okUtils.tableSuccessMsg(response.msg);
            ...
            function batchDel() {
                ...
                okLayer.confirm("确定要批量删除吗？", function (index) {
                    ...
                    let idsStr = okUtils.tableBatchCheck(table);
                    if (idsStr) {
                        okUtils.ajax("navigationLineDel", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
                            okUtils.tableSuccessMsg(response.msg);
            ...
            function batchRedis() {
                let idsStr = okUtils.tableBatchCheck(table);
                if (idsStr) {
                    okUtils.ajax("navigationLineBrushRedis", "post", {
                        id: idsStr,
                        _token: '{{csrf_token()}}'
                    }, true).done(function (response) {
                        okUtils.tableSuccessMsg(response.msg);
            ...
            function add() {
                json = JSON.stringify('');
                if (navigation_id > 0) {
                    okLayer.open("关联产品", "navigationLineBind/" + classify + '/' + navigation_id, "90%", "90%", null, function () {
                        navTable.reload();
                    })
                } else {
                    layer.msg("请先选择左侧导航, 然后进行关联线路", {icon: 5});
                    return;
                }
            }
            ...
            function edit(data) {
                ...
                okLayer.open("编辑产品", "navigationLine/" + data.id + "/edit", "50%", "50%", null, function () {
                    navTable.reload();
                })
            }
            ...
            table.on('edit(tableFilter)', function (obj) {
                okUtils.ajax("navigationLineTableEdit", "post", {
                    id: obj.data.id,
                    field: obj.field,
                    value: obj.value,
                    _token: '{{csrf_token()}}'
