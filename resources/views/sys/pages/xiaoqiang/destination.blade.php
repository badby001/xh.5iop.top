...
<script>
    layui.use(["admin", "element", "table", "form", "laydate", "okLayer", "eleTree", "okUtils", "okLayx", "introJs"], function () {
        ...
        function initPermissionTree() {
            okUtils.ajax("{{url('/sys/pages/system/xiaoqiang/destinationRead')}}", "get", null, true).done(function (response) {
                eleTree.render({
                    elem: '#permissionTree',
                    data: response.data,
                    @if(hasPower(237)) draggable: true, @endif
                    showLine: true,
                    expandOnClickNode: false,
                    renderAfterExpand: false,
                    defaultExpandedKeys: [0],
                    contextmenuList: [@if(hasPower(241))"add.async", @endif @if(hasPower(237))"edit.async", @endif @if(hasPower(239))"remove.async", @endif @if(hasPower(237)){
                        eventName: "set",
                        text: "设置"
                    }@endif],
                    indent: 10,
                ...
                eleTree.on("nodeDrag(data1)", function (d) {
                    okUtils.ajax("{{url('sys/pages/system/xiaoqiang/destinationTarget')}}", "post", {
                        'current': d.current.data.currentData,
                        'target': this[0]['dataset']['id'],
                        '_token': '{{csrf_token()}}'
                    }, true).done(function (response) {
                        //进行排序
                        okUtils.ajax("{{url('sys/pages/system/xiaoqiang/destinationSort')}}", "post", {
                ...
                eleTree.on("nodeAppend(data1)", function (d) {
                    ...
                    okUtils.ajax("{{url('sys/pages/system/xiaoqiang/destination')}}/", "post", {
                ...
                eleTree.on("nodeEdit(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okUtils.ajax("{{url('sys/pages/system/xiaoqiang/destinationNodeEdit')}}/" + d.data.id, "post", {
                            'title': d.value,
                            '_token': '{{csrf_token()}}'
                        }, true).done(function (response) {
                            // 异步操作必须调用之后才会修改
                ...
                eleTree.on("nodeRemove(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okUtils.ajax("{{url('sys/pages/system/xiaoqiang/destination')}}/" + d.data.id, "DELETE", {
                            '_token': '{{csrf_token()}}'
                        }, true).done(function (response) {
                            // 异步操作必须调用之后才会修改
                            d.async();
                ...
                eleTree.on("nodeSet(data1)", function (d) {
                    ...
                    if (d.data.id == 0) {
                        ...
                        okLayer.open("编辑导航", "destination/" + d.data.id + "/edit", "90%", "90%", null, function () {
                ...
                eleTree.on("nodeClick(data1)", function (d) {
                    ...
                    // console.log(d.node);    // 点击的dom节点
                    // console.log(this);      // 与d.node相同
                    $('#destination_id').val(d.data.currentData.id);
        ...
        function initPermissionTable(destination_id) {
            let navTable = table.render({
                elem: '#tableId',
                url: '{{url('/sys/pages/system/xiaoqiang/destinationLineRead')}}',
                where: {'destination_id': destination_id},
                limit: '{!! frame()['limit'] !!}',
                limits: [{!! frame()['limits'] !!}],
                title: '目的地关联线路列表_{{getTime(3)}}',
                page: true,
                even: true,
                toolbar: '<div class="layui-btn-container">\n' +
                    @if(hasPower(243)) '<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="batchEnabled">批量启用</button>\n' +
                    '<button class="layui-btn layui-btn-sm layui-btn-warm" lay-event="batchDisabled">批量停用</button>\n' + @endif
                        @if(hasPower(239)) '<button class="layui-btn layui-btn-sm layui-btn-danger" lay-event="batchDel">批量删除</button>\n' + @endif
                ...
                cols: [[
                    ...
                    {field: "destination_name", title: "目的地", width: 100, sort: true},
                    ...
                    {field: "up_time", title: "修改时间", width: 145, sort: true},
                    {
                        title: "操作", width: 100, align: "center", fixed: "right", templet: function (d) {
                            var del = "@if(hasPower(239))<a href=\"javascript:\" title=\"删除\" lay-event=\"del\"><i class=\"layui-icon\">&#xe640;</i></a>@endif";
                            if (d.is_lock == 1) {
                                return del;
                            } else {
                                return '';
            ...
            function batchEnabled() {
                okLayer.confirm("确定要批量启用吗？", function (index) {
                    ...
                    if (idsStr) {
                        okUtils.ajax("destinationStart", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
            ...
            function batchDisabled() {
                okLayer.confirm("确定要批量停用吗？", function (index) {
                    ...
                    let idsStr = okUtils.tableBatchCheck(table);
                    if (idsStr) {
                        okUtils.ajax("destinationStop", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
                            okUtils.tableSuccessMsg(response.msg);
            ...
            function batchDel() {
                ...
                okLayer.confirm("确定要批量删除吗？", function (index) {
                    ...
                    let idsStr = okUtils.tableBatchCheck(table);
                    if (idsStr) {
                        okUtils.ajax("destinationDel", "post", {
                            id: idsStr,
                            _token: '{{csrf_token()}}'
                        }, true).done(function (response) {
                            okUtils.tableSuccessMsg(response.msg);
            ...
            table.on('edit(tableFilter)', function (obj) {
                okUtils.ajax("destinationTableEdit", "post", {
                    id: obj.data.id,
                    field: obj.field,
                    value: obj.value,
                    _token: '{{csrf_token()}}'
