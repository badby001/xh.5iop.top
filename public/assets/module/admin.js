...
layui.define(['layer'], function (exports) {
    ...
    admin.events = {
        ...
        theme: function () {
            ...
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-theme', url: option.url || 'sys/pages/theme'
        ...
        note: function () {
            var option = admin.util.deepClone($(this).data());
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-note', url: option.url || 'sys/pages/note'
            }, admin.parseLayerOption(option)));
        },
        /* 打开消息 */
        message: function () {
            var option = admin.util.deepClone($(this).data());
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-notice', url: option.url || 'sys/pages/message'
            }, admin.parseLayerOption(option)));
        },
        /* 退出登录 */
        logout: function () {
    ...
    //压缩方法
    admin.dealImage = function (base64, w, callback) {
    ...
    admin.putb64 = function (picBase, myputb64, myqiniu, myUptoken) {
        /*picUrl用来存储返回来的url*/
        var picUrl;
        /*把头部的data:image/png;base64,去掉。（注意：base64后面的逗号也去掉）*/
        //picBase = picBase.substring(23);
        picBase = picBase.replace(/^data:image\/\w+;base64,/, '');
        ...
        //var url = myputb64 + fileSize(picBase);
        var url = myputb64 + '-1';
